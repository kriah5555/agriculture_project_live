...
def build_report_context(api_id):
    ...
    try:
        ...
        if lat and lon:
            import weather as wm, ndvi as nm
            import add_elevation_to_dataset as em, add_soilgrids_to_dataset as sm
            from map_utils import predict_for_point, dummy_predictions, classify_fertility, dummy_fertility
            TARGETS = ["ph","ec","n","p","k","organic_carbon","s","fe","zn","cu","b","mn"]
            lgbm = {}
            for t in TARGETS:
                pp = os.path.join(_MODEL_DIR, f"lgbm_{t}.pkl")
                if os.path.exists(pp):
                    with open(pp, 'rb') as f: lgbm[t] = pickle.load(f)
            rf = None
            rp = os.path.join(_MODEL_DIR, "rf_fertility.pkl")
            if os.path.exists(rp): rf = pickle.load(open(rp, 'rb'))
            if lgbm:
                ml_preds, env = predict_for_point(lat, lon, lgbm, wm.get_weather, nm.get_ndvi, em.get_elevation, sm.get_soil_texture)
                fertility = classify_fertility(ml_preds, env, rf)
            else:
                ml_preds, env = dummy_predictions(lat, lon)
