...
# ---------------------------------------------------------------------------
# Dummy fallbacks (used when models are not loaded) — real ICAR units
# ---------------------------------------------------------------------------
def dummy_predictions(lat, lon, seed_offset=0):
    ...
    # Dummy values use real ICAR-unit ranges
    preds = {
        "ph":             wave(5.5, 8.5),   # 0–14 scale, realistic 5.5–8.5
        "ec":             wave(0.1, 2.5),   # dS/m, NonSaline
        "n":              wave(80,  480),   # kg/ha
        "p":              wave(5,   30),    # kg/ha
        "k":              wave(80,  400),   # kg/ha
        "organic_carbon": wave(0.2, 2.0),   # %
        "s":              wave(4,   40),    # ppm
        "fe":             wave(2,   30),    # ppm
        "zn":             wave(0.2, 5.0),   # ppm
        "cu":             wave(0.1, 5.0),   # ppm
    ...
    features = {
        "ndvi":          wave(0.2, 0.8),
        "temperature_c": wave(15,  38),
        "rainfall_mm":   wave(400, 2500),
        "elevation_m":   wave(10,  1500),
        "clay_pct":      wave(10,  60),
        "sand_pct":      wave(10,  60),
        "silt_pct":      wave(10,  60),
    }
...
def dummy_fertility(lat, lon):
    rng = np.random.default_rng(int(abs(lat * 100 + lon * 100)))
    return rng.choice(["Low", "Medium", "High"], p=[0.3, 0.6, 0.1])
